...
const TourismGuideSection = () => {
  ...
  return (
    <section className="max-w-7xl mx-auto px-4 py-16">
      ...
      <Tabs>
        <TabList className="flex gap-6 mb-8 border-b ">
          <Tab className="cursor-pointer py-2 px-4 text-sm md:text-lg text-light border-b-2 border-transparent   hover:border-[#00A99D] focus:outline-none focus:border-[#00A99D]">
            Our Packages
          </Tab>
          <Tab className=" cursor-pointer py-2 px-4 text-sm md:text-lg text-light border-b-2 border-transparent hover:border-[#00A99D] focus:outline-none focus:border-[#00A99D]">
        ...
        <TabPanel>
          {loadingPackages ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {packages.map(pkg => (
                <div key={pkg._id} className="bg-p1 shadow-md rounded-lg overflow-hidden">
                  ...
                  <div className="p-5">
                    ...
                    <p className="text-light mb-4">Price: ${pkg.price}</p>

                    <Link to={`/packageDetails/${pkg._id}`}>
                      <button
                        className="px-4 py-2 cta-btn text-light rounded-full  transition text-sm  md:text-lg"
                      >
                        View Details
                      </button>
                    </Link>
        ...
        <TabPanel>
          {loadingGuides ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {guides.map(guide => (
                <div key={guide._id} className="bg-p1 shadow-md rounded-lg p-4 flex flex-col items-center text-center">
                  ...
                  <p className="text-light3 mb-4">Experience {guide.experienceYears} years</p>
                  <Link to={`/guides_details/${guide._id}`}>
                    <button
                      className="px-4 py-2 cta-btn text-light rounded-full hover:bg-[#00796b] transition text-sm md:text-lg"
                    >
                      View Details
                    </button>
                  </Link>
